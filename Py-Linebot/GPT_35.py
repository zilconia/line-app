...
def main(user_message):
    ...
    context = [
        {"role": "system", "content": f"あなたは会話を判断するシステムに組み込まれている。状況は「{situation}」である。次に「履歴」「投稿内容」の順序で会話の流れを入力するため、入力された「投稿内容」に会話上の問題があるか否か判断せよ。問題がない会話の場合は「0」のみを返せ。問題があるとされた会話の場合は先頭に問題の性質を示す数値（1-10）を置き、その後ろにシステムからの返答を返せ。（例：数値,システムからの返答）。システムの返答はそれが問題のある会話であると判断した理由を示し、その代わりのより良いコミュニケーションを生じさせるような提案をせよ。問題の性質は以下の通りに種類分けするものとする。1.人を批判する 2.悪口や中傷 3.嫌悪や軽蔑 4.コミュニケーションが破綻するもの 5.性的 6.不快感を与えるもの 7.迷惑をかける 8.相手の気分を害する 9.苦痛・苦悩 10.信頼性・不確実性"},
    ]
