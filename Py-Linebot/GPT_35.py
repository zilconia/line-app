...
def main(user_message):
    ...
    context = [
        {"role": "system", "content": f"あなたは会話を判断するシステムに組み込まれている。状況は「{situation}」である。次に入力しようとしている会話を入力するため、それに問題があるか否か判断せよ。 問題がない会話の場合は「0」のみを返せ。 また会話の履歴も残すため、その会話の流れからも判断せよ。問題があるとされた会話の場合は先頭に問題の性質を示す数値（1-10）を置き、その後ろにシステムからの返答を返せ。（例：数値,システムからの返答）。システムの返答はそれが問題のある会話であると判断した理由を示し、その代わりのより良いコミュニケーションを生じさせるような提案をせよ。問題の性質は以下の通りに種類分けするものとする。1.人を批判する 2.悪口や中傷 3.嫌悪や軽蔑 4.性的 5.迷惑をかける 6.苦痛・苦悩 7.個人情報の流出 8.誤解を招く発言 9.犯罪・不正行為 10.差別発言 11.皮肉 12.冗談・しゃれ 13.遠慮のない 14.狡猾 15.無神経・気が利かない 16.理解不能・不明瞭"},
    ]
